...
<script type="text/javascript">alert("Warning! If you edit a form that is currently in use by a campaign/action page, you may confuse the data that is being tracked!");</script>	
...
if(isset($_REQUEST['action'])) {
	if( ($_REQUEST['action']=='prebuilt' || $_REQUEST['action']=='existing' || $_REQUEST['action']=='scratch') && isset($_REQUEST['formID']) ) {
		...
		if($_REQUEST['action']=='existing') {
			...
			"fieldrequired, fieldsize, fieldmaxsz, fieldrows, fieldtext, fieldoptions, name " . 
			"FROM forms " . 
			"WHERE account_id='{$_SESSION['account_id']}' AND id='$formid'";
		} elseif ($_REQUEST['action']=='prebuilt') { //go look up prebuilt form:
			$q = "SELECT fontsize, fonttype, fontcolor, fontbackground, buttonlabel, fieldorder, fieldtype, " . 
			"fieldrequired, fieldsize, fieldmaxsz, fieldrows, fieldtext, fieldoptions, name " . 
			"FROM forms " . 
		...
		if (mysqli_num_rows($r) > 0) {
			while ($row = mysqli_fetch_array($r, MYSQLI_ASSOC)) {
				...
				$oldformname = $row['name'];
			}
			
				echo '<p><b>Drag Fields Over to Build Your Form!</b></p>	
			<table border = "0"><tr><td>Name of Form:</td><td><input type="text" width="250px" id="formname" readonly="readonly" value="' . $oldformname . '" /></td><td>
			<input type="button" value="Save Form" onclick="itemize();" /></td></tr></table>
			<br /><div id="results"></div>
			...
			echo '<p><b>Drag Fields Over to Build Your Form!</b></p>	
			<table border = "0"><tr><td>Name of Form:</td><td><input type="text" width="250px" id="formname" readonly="readonly" /></td><td>
			<input type="button" value="Save Form" onclick="itemize();" /></td></tr></table>
			<br /><div id="results"></div>
...
function moveItem( draggable,droparea){
	...
	itemize = function() {
		...
		if(formname.length==0) {
		    ...
		    location.href="saveform.php?" + formdata+'&order='+order+'&type='+type+'&req='+req + '&sz='+sz+'&maxsz='+maxsz+'&rows='+rows+'&txt='+txt+'&opt='+opt+'&formID='+formID;
