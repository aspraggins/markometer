...
if (isset($_POST['submitted']))
{
    ...
    $goalcheck = $cname = $datecheck = $namecheck = $subjectcheck = $costcheck = $typecheck = FALSE;
...
<div class="alert-holder">
	...
	if(substr($trimmed['beginDate'],0,5)!='Start' && date('m/d/Y', strtotime($_SESSION['campaign_begin']))!='12/31/1969') {
		$datecheck = TRUE;
	}
...
<div class="alert-holder">
	<div class="alert-box red">
		<div class="holder">
			<div class="frame">
				<a class="close" href="#">X</a>
				<p>Please enter valid start and end dates for your campaign.</p>
			</div>
		</div>
	</div>
</div>';
		...
		<div class="alert-holder">
			<div class="alert-box red">
				<div class="holder">
					<div class="frame">
						<a class="close" href="#">X</a>
						<p>Please select a Campaign Type for your campaign.</p>
					</div>
				</div>
			</div>
		</div>';		
	...
	if($namecheck && $datecheck && $goalcheck && $subjectcheck && $costcheck && $typecheck) {
		// Move to next page in campaign startup:
		$url = '/build/campaigns/new/step2/';
		header("Location: $url");
	}
}
?>
...
<form action="#" method="post" id="new_campaign_step1" name="campform" class="steps-form">
<fieldset>
...
<div class="row">
	...
	<div class="steps-holder">
		<div class="heading">
			<h2>Campaign Type</h2>
			<?php # Help Button
			$helphref = 'build-campaigns-type.html';
...
echo $_SESSION['campaign_subject']; ?>" />
	</div>
</div>
	...
	<div class="steps-holder">
		...
		<div class="calendar-box">
			<div class="calendar">
				...
				<?php 
					$datestring = 'Start (Required)                         ' . date('m/d/Y');
					if (isset($_SESSION['campaign_begin'])) {
						if (date('m/d/Y', strtotime($_SESSION['campaign_begin']))!='12/31/1969') {
							$datestring = date('m/d/Y', strtotime($_SESSION['campaign_begin']));
						}
					}
					echo 'value="' . $datestring . '"';
			...
			<div class="calendar">
				...
				<?php 
					$datestring = 'End (Not Required)                          ' . date('m/d/Y');
					if (isset($_SESSION['campaign_end'])) {
						if (date('m/d/Y', strtotime($_SESSION['campaign_end']))!='12/31/1969') {
							$datestring = date('m/d/Y', strtotime($_SESSION['campaign_end']));
						}
					}
...
<div class="coll">
	<div class="heading">
		<?php
			...
			if (isset($_SESSION['leadimport'])) {
				if ($_SESSION['leadimport']=='Y') $importcheckbox = TRUE;	
			}
		?>
		<input tabindex="8" type="checkbox" name="import" id="import" class="check" <?php if ($importcheckbox) echo ' checked="checked" ';?>/>
		<label tabindex="9" for="import">One-Time Lead Import <span>(for past campaigns)</span></label>
			<?php # Help Button
			$helphref = 'build-campaigns-import.html';
...
<div class="content-link">
	<div class="link-holder">
		<div class="link-frame">
			<a href="#" class="btn" onclick="document.getElementById('new_campaign_step1').submit(); return validateAll()" /><span>Save &amp; Go To Next Step &gt;</span></a>
			<em>or</em>
