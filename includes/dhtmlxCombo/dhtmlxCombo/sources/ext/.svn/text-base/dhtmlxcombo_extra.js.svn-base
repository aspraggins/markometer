...
dhtmlXCombo_checkboxOption.prototype.render = function(){
    if (!this.content) {
        ...
        this.content.innerHTML=html;
        this.content.firstChild.onclick = function(e) {(e||event).cancelBubble=true; 
        	this.parentNode.parentNode.combo.setCheckedByValue(this.parentNode._self.value,this.checked);
...
dhtmlXCombo_checkboxOption.prototype._DrawHeaderCheckbox = function(self, name, width)
{
	...
	z.style.height="auto";
    z.style.visibility = 'hidden';
    z.onclick = function(e) {(e||event).cancelBubble=true; self.setCheckedByValue(self.getActualValue(),this.checked); }
    self.DOMelem.appendChild(z);
...
dhtmlXCombo.prototype.setChecked=function(index,mode){
	...
	if (this.getActualValue()==this.optionsArr[index].value)
		this.DOMelem_checkbox.checked=(!(mode===false));
}
