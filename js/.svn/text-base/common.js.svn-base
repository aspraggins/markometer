...
// Calendar JavaScript
	...
	function CalendarCheck() { 
		var selection = document.getElementById('TableAction').value;
...
// Check/Uncheck Input Boxes Toggle JavaScript
...
// Enable Agent Name Textbox on Individual Lead Page

function enableagentname() {
	var x=document.getElementById("channel")
	if(x.value=="agent"||x.value=="dist") {
		document.moreinfoform.agentname.disabled=false;
	}else{
		document.moreinfoform.agentname.disabled=true;
	}
}

// Confirm Delete Popup on Individual Lead Page

function confirmdelete() {
	if(document.moreinfoform.deletelead.checked) {
		alert('Please uncheck the Delete option unless you really mean to delete this lead!');
	}
}

// Show/Hide (for Private Labeling)

var state = 'none';

function showhide(layer_ref,request) {
	state = 'block';
	if (request == 'close') { 
		state = 'none'; 
	} 
