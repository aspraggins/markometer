...
<a href="/help/create-action-page-choose.html" class="btn-help" title="Help" onclick="Modalbox.show(this.href, {title: this.title, width: 600, evalScript: true}); return false;"><span>Help?</span></a>
</div>
...
if (isset($_POST['submitprebuilt'])) {
	if(isset($_POST['chooseprebuilt'])) {
		...
		$url = '/create/actionpage/new/?action=prebuilt&id=' . $chosen;
...
if (isset($_POST['submitexisting'])) {
	if(isset($_POST['chooseexisting'])) {
		...
		$url = '/create/actionpage/new/?action=existing&id=' . $chosen;
		if(isset($_POST['campaignseq'])) {
			$url .= '&camp=' . $_POST['campaignseq'];
		}
		header("Location: $url");
...
<p class="head-caption">If these options don't tickle your fancy we'll be glad to design it for you (one-time $399 charge). <a href="http://www.wikipedia.org">More information &gt;</a></p>
...
<div class="content-link">
	<div class="link-holder">
		<div class="link-frame">
			<a class="btn" href="/create/actionpage/new?action=scratch<?php if($campaignid) echo '&camp=' . $campaignid; ?>"><span>Choose &amp; Edit &gt;</span></a>
		</div>
	</div>
</div>
