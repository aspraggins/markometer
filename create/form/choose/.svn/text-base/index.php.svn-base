...
$returnurl = '/login/?returnpage=create/form/choose';
...
if (isset($_POST['submitprebuilt'])) {
	if(isset($_POST['chooseprebuilt'])) {
		...
		if($chosen==0) {
			$url = '/create/form/new?formID=new&action=scratch';
		} else {
			$url = '/create/form/new/?action=prebuilt&formID=' . $chosen;
		}
...
if (isset($_POST['submitexisting'])) {
	if(isset($_POST['chooseexisting'])) {
		...
		$url = '/create/form/new/?action=existing&formID=' . $chosen;
		if(isset($_REQUEST['actionid'])) {
			$url .= '&actionid=' . $_REQUEST['actionid'];
		}
		header("Location: $url");
...
<div class="content-link">
	<div class="link-holder">
		<div class="link-frame">
			<a class="btn" href="/create/form/new?formID=new&action=scratch<?php if($actionid) echo '&actionid=' . $actionid; ?>"><span>Choose &amp; Edit &gt;</span></a>
		</div>
	</div>
</div>
