<?php # add quote from within a modal box, 2nd box
...
if (isset($_GET['submitted'])) {
	...
	if(strlen($_GET['name'])>0 && strlen($_GET['amount'])>0) {
		...
		$q = "INSERT INTO quotes (name, number, amount, description, date_created, lead_id) " . 
		"VALUES('$name', '$number', '$amount', '$description', NOW(), '{$_SESSION['idlead']}')";

		$r = mysqli_query($dbc, $q) or trigger_error("Query: $q\n<br />MySQL Error: " 
		. mysqli_error($dbc));
		
		if (mysqli_affected_rows($dbc) == 1) {
			//set "Quoted" flag in lead record:
			$q = "UPDATE leads SET stage='QT', new='N' WHERE idlead='{$_SESSION['idlead']}'";

			$r = mysqli_query($dbc, $q) or trigger_error("Query: $q\n<br />MySQL Error: " 
			. mysqli_error($dbc));
		
			//build lead info for note insert:
			$note = 'Quote created.\n';
			...
			echo 'Your quote was added and recorded in the notes for this lead.';			
		}
	} else {
